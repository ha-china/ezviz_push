...
- 传感器：电池电量、WiFi信号、SD卡健康度/容量/状态/首次录像时间、充电状态、屏幕亮度、省电模式、麦克风音量、门卡数量、报警时间（含按报警类型的独立时间）、呼叫时间、呼叫动作（响铃/接听/挂断）、人脸识别ID、报警类型、最近事件、最后活动时间
- 开关：在线状态、呼叫按铃、报警触发、布防状态、移动检测开关（含人形/人脸识别/徘徊/陌生人等分类型检测开关）、夜灯、静音
- 摄像头：呼叫封面图片、报警图片（按报警类型独立实体，互不覆盖）
- 补充信息以实体属性呈现：报警ID/通道名/位置、设备公网 IP、布防计划表、电源状态等
- 设备信息重启后持久化保留
